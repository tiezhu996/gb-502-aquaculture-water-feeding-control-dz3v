...
func (s *PlanService) Revoke(id uint, reason string, actor Actor) (model.FeedingPlan, error) {
	...
	if _, err := s.recommendations.MarkPlanInvalid(plan.ID, InvalidReasonPlanRevoked, time.Now().UTC()); err != nil {
		return model.FeedingPlan{}, WrapError(CodeInternal, "失效相关投喂建议失败", err)
	}
...
func (s *PlanService) Delete(id uint, actor Actor) error {
	...
	recCount, err := s.recommendations.CountForPlan(id)
	if err != nil {
		return WrapError(CodeInternal, "检查投喂建议失败", err)
	}
	if recCount > 0 {
		return NewError(CodeConflict, "计划已生成投喂建议快照，不能删除")
	}
